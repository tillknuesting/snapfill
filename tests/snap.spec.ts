...
test.describe('UI guards', () => {
  ...
  test('@mobile mobile toolbar uses the shared PDF actions', async ({ page }) => {
    await openFixture(page, /IRS 1040 \(2022\)/)
    ...
    await expect(page.getByRole('button', { name: /^Pages$/ })).toBeEnabled()
    await expect(page.getByRole('button', { name: /^Redact$/ })).toBeEnabled()
    await expect(page.getByRole('button', { name: /^Add image/ })).toBeEnabled()
    await expect(page.getByRole('button', { name: /^Watermark$/ })).toBeEnabled()
    await expect(page.getByRole('button', { name: /^Page numbers$/ })).toBeEnabled()

    await page.getByRole('button', { name: /^Merge another PDF$/ }).click()
    const popover = page.locator('[data-slot="popover-content"]').last()
    await expect(popover).toBeVisible()
    await expect(popover.getByRole('button', { name: /^Add to start$/ })).toBeVisible()
